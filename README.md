...
```shell
sudo pacman-key --recv-keys 04FE978DDA00B47B
```

**Step 2.** It's recommended to verify the fingerprint.

```shell
sudo pacman-key --finger 04FE978DDA00B47B
```

**Step 3.** Locally sign the key.

```shell
sudo pacman-key --lsign-key 04FE978DDA00B47B
```
...
| `crystal` | x86_64        | Desktop-independent graphical login manager for X11                                         |
| `meris`   | x86_64        | Simple terminal emulator for Wayland and X11 with OpenGL rendering and minimal dependencies |
